...
# Each equation is set up and allocated to the matrix, on the left side of the equation, the coefficients of the variables on the {var} vector are inputed in an array and allocated to the respective line on the matrix A.
# Entries are done such as if everything was kept on one side of the equation -- right notation is not what it seems like (i.e its on the right), it is just the set of terms that are not affected by an unknown variable, ie: u({vec},x) = u_left({vec},x) + u_right(x). On the right side are the non-homogeneous terms (known scalars) from your compatibility/boundary equations. 
...
w_left      = lambda X : -1/(E*Iyy)  *  np.array([0,1/6*MC(X,x1,3), 0, 1/6*MC(X,x2,3), 0, 1/6*MC(X,x3,3), 1/6*MC(X,x3,3), 0, -cos(theta)/6*MC(X,x_I,3), 0, 0, X, 1, 0])  
w_right       = lambda X : -1/(E*Iyy_airfoil)*         (1/6*MC(X,x_II,3)*P*cos(theta))                                                                                 
                                                                             

phi_left   = lambda X : 1/(G*J)           * np.array([(z_h)*MC(X,x1,1), 0, (z_h)*MC(X,x2,1), 0, (z_h)*MC(X,x3,1), 0, -cos(theta)*ha/2*MC(x,x_I,1), 0, 0, 0, 0, 1])
phi_right   = lambda X : 1/(G*J)           *         ( P*(m.sin(theta)*eta*MC(X,x_II,1)  - cos(theta)*ha/2*MC(X,x_II,1))   + T_q_II(X) )


T_left      = lambda X :                     np.array([(z_h)*MC(X,x1,0), 0,-(z_h)*MC(X,x2,0), 0, -(z_h)*MC(X,x3,0), 0, m.sin(theta)*eta*MC(X,x_I,0)-cos(theta)*ha/2*MC(X,x_I,0), 0, 0, 0, 0, 0])
T_right      = lambda X :                             ( - P*(m.sin(theta)*eta*MC(X,x_II,0) - cos(theta)*ha/2*MC(X,x_II,0)))
## Unknowns  :  {vec} = [Ry1, Ry2, Ry3, Rz1, Rz2, Rz3, Cu_p0, Cu0, Cv_p0, Cv0, Ctheta0, Py_I, Pz_I]  
 ## Variables :  {var} = [Ry1, Rz1, Ry2, Rz2, Ry3, Rz3, R_I, C1, C2, C3, C4, C5] 
My_left      = lambda X :                     np.array([0, MC(X,x1,1), 0, MC(X,x2,1), 0, MC(X,x3,1), -cos(theta)*MC(X,x_I,1), 0, 0, 0, 0, 0])                              
My_right      = lambda X :                             ( P*cos(theta)*MC(X,x_II,1))                                                                                     
